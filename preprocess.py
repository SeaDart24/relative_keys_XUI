...
def pre(data_df,multiclass, model_num = 5):
    ...
    best_model_json = pickle.dumps(best_model)
    ...
    ##Below is for creating res_dict
    ...
    if multiclass:
        return best_model_json,[], train, test,res_dict


    ##BELOW IS FOR CREATING MULTIPLE MODELS

    #alg_dict = alg_config_parse('config.yaml')            
    #datasetsname = alg_dict['datasetsname']
    #model_num = alg_dict['model_num']

    data_df = train #pd.read_csv('data_process/'+datasetsname+'_train.csv', index_col=0)
    data_df.reset_index(drop=True, inplace=True)
    X_train = data_df.iloc[:, :-1]  
    y_train = data_df.iloc[:, -1]   

    #Best-model's params and generate new models 
    ...
    return best_model_json,models_json, train, test,res_dict
